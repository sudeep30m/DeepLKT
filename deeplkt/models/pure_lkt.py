...
class PureLKTNet(LKTLayers):
    ...
    def forward(self, img_i):
        ...
        # print("Image i = ", img_i)
        # img_quad = x[2]
        p_init = torch.zeros((BATCH_SIZE, 6), device=self.device)
        # print(img_i.shape)
        # print(img_tcr.shape)
        # print(p_init.shape)
        ...
        img_quad = img_quad.repeat(BATCH_SIZE, 1)

        quad = img_quad
        omega_t = self.form_omega_t(coords, BATCH_SIZE)
        ...
        omega_t = torch.cat((omega_t, torch.ones((BATCH_SIZE, N, 1), device=self.device)), 2)  # (B x N x 3)
        ...
        while(self.params.max_iterations > 0):
            ...
            # print(r)
            r = r.permute(0, 2, 3, 1)            
            r = r.contiguous().view(BATCH_SIZE, C * h * w, 1)
            # analyse_output(r)
            # print(r.norm())
